...
下午做了一次桂林，感觉D和G都可以补一下，J看出是一道后缀数组，但是没想好具体思路。
...
晚上补了昨天没有做出来的D，其实一开始思路很接近，但是没有及时找到Hack数据就一直卡着思路，后来写了一个样例卡掉了代码，然后发现他限定的$\frac{n(n - 1)}{2}$其实是有道理的，我们交换的时候需要不改变其他数的相对位置，就可以理解为一个一个往前面移动，最坏的情况就是$\frac{n(n - 1)}{2}$。
...
上午上课，下午看了一下威海的D，发现又是一个没见过的知识点`border`，不过好像也不是很难懂，就是需要知道一些性质：$len - border$既是字符串的周期。
